...
def variable_information_html(gdf):
    ...
    for cols in numeric_columns:
        ...
        at = AnchoredText(
            anchorText, prop=dict(size=10), frameon=True, loc="upper right"
        )
        g2.add_artist(at)
        # LISA Scatterplot
        ...
        gdf.plot(
            ...
            scheme="quantiles",
            legend=True,
            legend_kwds={"loc": "best", "title": cols, "fmt": "{:.0f}"},
        )
        # Equal Intervals
    ...
    for cols in numeric_columns:
        ...
        div_string += str(
            '<div id="'
            + cols
            + '" class="tabcontent table-responsive"><img src="data:image/png;base64,'
            + image_array[count]
            + '"></div>\n'
        )
        count = count + 1
