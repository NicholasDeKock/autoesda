...
def _summary_page(dataset, report_name):
    _summary_page_string = """
<div id="Summary" class="tabcontent">
<div class="grid-container">
    <div class="grid-item item-study-area">
      <h2>"""+ report_name + """</h2>
      <img src="data:image/png;base64,""" + _encode_image(dataset.study_area_figure()) + """">
    </div>
    <div class="grid-item item-dataset-overview">
      <h2>Dataset Overview</h2>
      """ + dataset.overview_statistics().T.to_html(border="1", classes="dataframe mystyle") +"""
    </div>
    <div class="grid-item item-descriptive-statistics">
      <h2>Descriptive Statistics</h2>
     """ + dataset.dataset_statistics().T.to_html(border="1", classes="dataframe mystyle") + """
    </div>
    <div class="grid-item item-sample">
      <h2>Dataset Sample</h2>
      """ + dataset.dataset_sample().to_html(border="1", classes="dataframe mystyle") + """
    </div>
  </div>
</div>"""
...
def _variable_information_page(dataset):
    _tab_string = ""
    _variable_information_page_string = ""

    for variable in dataset.numeric_variables():
        _tab_string += str("""<button class="tablinks" onclick="openTab(event, '""" + variable.name + """')">"""+ variable.name +"""</button>\n""")
        _variable_information_page_string += ("""
<div id='"""+ variable.name +"""' class="tabcontent">
<div class="grid-container">
  <div class="grid-item item-variable-statistics">
    <h2>Variable Statistics</h2>
""" + variable.variable_statistics().T.round(3).to_html(border="1", classes="dataframe mystyle") + """
  </div>
    <div class="grid-item item-variable-descriptive">
    <h2>Descriptive Figures</h2>
    <img src="data:image/png;base64,""" + _encode_image(variable.descriptive_figure()) + """">
  </div>
  <div class="grid-item item-local-moran">
    <h2>Local Moran's I</h2>
    <img src="data:image/png;base64,""" + _encode_image(variable.local_moran_figure()) + """">
  </div>
  <div class="grid-item item-local-geary">
    <h2>Local Geary's C</h2>
    <img src="data:image/png;base64,""" + _encode_image(variable.local_geary_figure()) + """">
  </div>
  <div class="grid-item item-choropleth">
    <h2>Choropleth Maps</h2>
    <img src="data:image/png;base64,""" + _encode_image(variable.choropleth_figure()) + """">
  </div>
</div>
</div>
        """)
    return _tab_string, _variable_information_page_string
...
def generate_report(input_data, report_title="Study Area", filename="autoESDA-vector-report"):
   ...
   summary_page_string = _summary_page(dataset, report_title)
   variable_information = _variable_information_page(dataset)
   variable_information_tab_string = variable_information[0]
   variable_information_page_string = variable_information[1]
...
.item-variable-descriptive {
  grid-column: 2;
  grid-row: 1;
}

.item-variable-statistics {
  grid-column: 1;
  grid-row: 1;
}
...
<!-- SUMMARY PAGE -->
""" 
+ summary_page_string + variable_information_page_string +                   
"""
